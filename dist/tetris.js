...
var drawBox = function(ctx,colors,x,y,gridSize,radius = 2){
			if (y<0){
				return;
			}
...
var counterCounts = 90;

var gameOverTime = counterCounts * 1000;
//scene gradient start color 
var sceneBgStart = 'black';

//scene gradient end color 
var sceneBgEnd = 'black';
...
exports.GAMEOVER_TIME = gameOverTime;
exports.COUNTER_COUNTS = counterCounts;
...
var calcRewards = function(rows){
	if (rows&&rows.length>1){
		return Math.pow(2,rows.length - 1);	
	}
	return 0;
};
...
var calcScore = function(rows){
	...
	console.log(this.level,rows.length,'this.level,rows.length');
	if (rows&&rows.length){
		return this.level * 2 * rows.length;
	}
	return 0;
};
...
var calcIntervalByLevel = function(level){
	return consts.DEFAULT_INTERVAL  / (1.5 ** level);
};
...
Tetris.prototype = {
	...
	_update:function(){
		...
		if (currentTime - this.startTime > consts.GAMEOVER_TIME){
			stopCounterInterval();
	...
	_check:function(){
		...
		if (rows.length){
			...
			var score = this.level * 2 * rows.length;
